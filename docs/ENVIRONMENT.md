...
### `GEOAPIFY_API_KEY`
- **Type:** server-only **(secret)**
- **What it is:** the key for Geoapify, the default place-search provider behind
  Eden's restaurant/venue tool. Free tier, no credit card to start.
- **Where to find it:** create a project at <https://myprojects.geoapify.com>
  and copy its API key.
- **Enables:** the `places.search` tool (finding restaurants near a location).

### `EDEN_PLACES_PROVIDER`
- **Type:** server-only
- **What it is:** which registered place-search provider Eden should use.
- **Default:** `geoapify`.
- **When to set it:** once another provider (e.g. Google Places) is registered
  and you want to switch — no code change, just this value plus that provider's key.

### `EDEN_DEFAULT_LOCATION`
- **Type:** server-only
- **What it is:** a fallback location used when a request doesn't name one, e.g.
  `Christchurch, New Zealand`.
- **Default:** none (Eden asks for an area when none is given).
- **When to set it:** set it to your usual area so requests like "Italian dinner
  tonight" search immediately without asking where.
